...
![section 1 image](DocImages/image.png)
...
![weapon](DocImages/image-13.png) ![gun weapon](DocImages/image-14.png)
In the process of going right, the player will get a weapon, the weapon is a close combat weapon, players need to get close to the enemy to defeat him. There are 2 types of weapons that players can get in the game, one is a close combat weapon which is one that player can get in the first section, the second one will be a ranged weapon, which is a gun, the players will get a ranged weapon later in the game. 

![first scene](DocImages/image-17.png)
After Getting the weapon,the player will encounter the first enemy called Chomper, Chomper is a close combat enemy, after the player goes at the range, Chomper will start to get closer and try to attack the player. Player will have 5 hp at the start if the Chomper did attack the player, the player will lose 1 hp, when the player hits the Chomper once, the Chomper will die. This part is for the player to experience how to attack the enemy, which will encounter more enemies on the way.

![player push the box to the right](DocImages/image-15.png) ![player standing between the acid](DocImages/image-16.png) 
After that, the player has to push the box to the right wall, and then the player can jump over the wall and then reach two acids. 
![2 acid](DocImages/image-18.png) 
The player will jump over one of them to reach another land before jumping over the second acid. the player will then move forward and find a save point.
![moving platform and acid](DocImages/image-19.png)
There is another acid at the front, but it is very big. The player needs to jump on the moving platform above to pass the acid. In this part the player is to experience how to jump though the acid and how to use a moving platform to go through the acid, there will be a lot of acid that the player needs to go though.

![2 way image](DocImages/image-20.png)
...
![area where the key at](DocImages/image-21.png)
Inside the cave. There is acid, 1 Spitter and a new trap for the player which is Spikes, Spikes will cause the player to lose 1 hp. jumping all over and killing Spitter, 
![unknow trap](DocImages/image-22.png)
Player cannot very see what's below, there will be 1 Spikes at the right and are acid ar the left, If the player jumps without thinking of anything, he may hit a Spikers or go in thought the acid and die. This can be regarded as a Discovery, Drama and Exploration, because players don’t know what will be next and want to find out and die.

![getting the key and getting out the cave](DocImages/image-23.png)
...
![3 enemy and spikes](DocImages/image-24.png)
There are 2 enemies and 1 Spikes there, this is a bit of a challenge, the player will have to run to a good position and jump higher than they can to pass through the spikes without getting any damage. After all, this player finally can go to the next section.

![checkpoint and 2 health pick up](DocImages/image-25.png)
...
![second key area](DocImages/image-28.png)
...
![a way to section 3](DocImages/image-29.png) ![A are in section 3](DocImages/image-30.png)
Jumping over the acid and the spikes in the middle, the player is about to enter the third section, there are many mechanisms and puzzles in this level that players need to explore and trigger.
![tigger door but it is lock](DocImages/image-31.png)
The level leading to the third key is blocked by the trigger door, player can only go down to find the switch that can open the door, 
![Part of the area in the section 3 cave](DocImages/image-32.png)
To move to the left below, the player needs to move to the right first and turn on the button. 
![area to turn on the moving platform](DocImages/image-33.png)
Inside the right side there are acid spikes and 3 enemies are waiting for the player. There are two lands for players inside in acid, but these two lands are smaller than the previous ones and there are monsters on them, and spike at the end of the acid, this also tests the player skills and flexibility.

![final area inside the cave](DocImages/image-34.png)
After the switch has been touched, the platform on the left will start moving, so the player can go through it. The player needs to touch the switch below first to move the bridge on the left to the left and let the box fall. Then the pressure pad can be pressed out, which can open the trigger door and make the platform move so the player can go back to the trigger door.
...
### 1.1. Discovery
![section 1](DocImages/image-39.png)
...
### 1.2. Drama
![first key area](DocImages/image-40.png) ![second key area](DocImages/image-41.png)![third key area](DocImages/image-42.png)
...
![2 way in section 1](DocImages/image-43.png) 
![2 way in section 2](DocImages/image-44.png)
...
![storyboards for first area](DocImages/image-1.png)
...
### 2.1. Acid
![storyboards for the 2 acid](DocImages/image-2.png)
...
![sstoryboards showing moving platforms and spitter](DocImages/image-3.png)
...
### 2.8. Spikes
![storyboards for spikes](DocImages/image-4.png)
...
![key area storyboard](DocImages/image-5.png)  ![key area storyboard](DocImages/image-6.png)
...
![storyboard for gun and health pick up](DocImages/image-7.png)
...
![Molecule Diagram](DocImages/image-8.png)

### 3.2. Level Map – Section 1
![Section 1 Level Map](DocImages/image-10.png)
### 3.3.	Level Map – Section 2
![Section 2 Level Map](DocImages/image-11.png)
### 3.4.	Level Map – Section 3
![Section 3 Level Map](DocImages/image-12.png)
...
### Tool Used: draw.io
**Nature of Use** To making storyboards.


**Evidence Attached?** ![draw.io](DocImages/image-37.png)


**Additional Notes:** I use draw.io to making storyboards for my level. i find draw.io is more easy and faster way for me to making a storyboards, and the important thing is, it is free.

### Tool Used: Goodnotes
**Nature of Use** to draw Level Map.


**Evidence Attached?** ![goodnotes](DocImages/image-38.png)


**Additional Notes:** I use Goodnotes to draw a Level Map for my level. The reason I use this is because I use this software very often, and I find out I can also draw things on this software so I decided to use it to make my level map.
